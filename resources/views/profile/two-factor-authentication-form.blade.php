            ...
            @if ($showingQrCode)
                ...
                @if ($showingConfirmation)
                    <div class="mt-4">
                        ...
                        <x-jetstream.input id="code" type="text" name="code" class="block mt-1 w-1/2"
            ...
            @else
                @if ($showingRecoveryCodes)
                    <x-jetstream.confirms-password wire:then="regenerateRecoveryCodes">
                        <x-jetstream.secondary-button class="me-3">
                            {{ __('Regenerate Recovery Codes') }}
                        </x-jetstream.secondary-button>
                    </x-jetstream.confirms-password>
                @elseif ($showingConfirmation)
                    <x-jetstream.confirms-password wire:then="confirmTwoFactorAuthentication">
                        <x-jetstream.button type="button" class="me-3" wire:loading.attr="disabled">
                            {{ __('Confirm') }}
                        </x-jetstream.button>
                    </x-jetstream.confirms-password>
                @else
